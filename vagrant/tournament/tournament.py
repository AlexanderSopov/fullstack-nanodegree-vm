...
def playerStandings():
    ...
    c = conn.cursor()
    c.execute('''
    select winners.idx, winners.namex, winners.wins, winners.wins + losers.losses as matches

    from 
    (select players.id as idx, players.name as namex, count(matches.winner) as wins
    ...
    on winners.idx = losers.id
    order by winners.wins desc
    ;''')
    #count(matches.winner) + count(matches.loser) as totalmatches
